...
|------|-------------|---------|
| `train_users_2.csv` | 213,451 users with destination labels | Download from Kaggle |
| `test_users.csv` | 62,096 unlabelled users | Download from Kaggle |
| `countries.csv` | Destination country metadata | Included in repo |
| `age_gender_bkts.csv` | Population data by age/gender | Included in repo |
...
The UI also displays the selected model's accuracy and NDCG@5 score for context.
...
- **Gradient Boost trained on bookers only:** By dropping NDF users before training, the model cannot predict the most common outcome (58.3% of users). It's useful for predicting *where* a booker goes, but not *whether* they'll book.
